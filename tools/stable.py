...
SCHEMA = {
    ...
    0x12: [("~", 2), ("~", 20)],
    0x13: [("~", 20)],
    0x14: ["u2", "s", TYPEDEF_OR_REF],
    0x15: [("~", 2), ("~", 23)],
    0x16: [("~", 23)],
    0x17: ["u2", "s", "b"],
    0x18: ["u2", ("~", 6), HAS_SEMANTICS],
    0x19: [("~", 2), METHOD_DEF_OR_REF, METHOD_DEF_OR_REF],
    0x1A: ["s"],
    0x1B: ["b"],
    0x1C: ["u2", MEMBER_FORWARDED, "s", ("~", 26)],
    0x1D: ["u4", ("~", 4)],
...
class Tables:
    ...
    def token(self, value):
        ...
        if table == 0x0A:
            ...
            owner = "?"
            if kind == 1 and 0 <= index < len(self.read(0x01)):
                tr = self.read(0x01)[index]
                owner = f"{self.string(tr[2])}.{self.string(tr[1])}".lstrip(".")
            elif kind == 0 and 0 <= index < len(self.read(0x02)):
...
SMALL_INT = {op: op - 0x16 for op in range(0x16, 0x1F)}
SMALL_INT[0x15] = -1
