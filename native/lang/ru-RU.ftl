app-name = Dossier

setting-up = Настройка
checking = Проверка
everything-works = Всё работает!
n-of = { $n } из { $total }

step-language = Язык
step-folder = Папка osu!
step-device = Это устройство
step-bot = Бот

language-why = Выбор языка влияет на весь отображаемый текст в приложении.
continue = Продолжить
back = Назад

folder-found = Нашлась на этом устройстве!
folder-found-two = На этом устройстве две. Переключатели позволяют выбрать нужный клиент.
folder-looking = Ищу osu! на этом устройстве…
folder-missing = На этом устройстве osu! не нашлась...
folder-missing-note = Приложение может хранить карты, скины и реплеи в своей папке.
folder-own = Приложение хранит всё в своей папке.
use-this = Взять эту
use-both = Взять обе
add-another = Добавить ещё…
browse = Обзор…
own-folder = Хранить всё в папке приложения
maps-from-mirrors = Карты берутся с зеркал
skins-exported = скинов, экспорт
maps-label = { $n ->
    [one] карта
    [few] карты
   *[many] карт
}
skins-label = { $n ->
    [one] скин
    [few] скина
   *[many] скинов
}
replays-label = { $n ->
    [one] реплей
    [few] реплея
   *[many] реплеев
}

device-why = Имя воркера который будет показывать бот.
device-name = Имя

bot-why = Отсканируйте или откройте Telegram здесь и подтвердите авторизацию.
code-cap = Код · 10 минут
open-telegram = Открыть Telegram
waiting-telegram = Жду Telegram…
linked = Привязано к боту
linked-to = Привязано к @{ $who }
later = Не хочу привязываться...
no-pairing-yet = Бот пока не умеет привязывать по коду. Попросите код у него.

check-folder = Папка osu!
check-ffmpeg = ffmpeg
check-engine = Движок
check-bot = Бот
not-installed = Не установлен
ffmpeg-why = Нужен для рендеринга реплеев.
where-to-get = Где взять
check-again = Проверить снова
continue-anyway = Продолжить без него
open-dossier = Открыть Dossier
asking-bot = Спрашиваю бота…
same-build = Последняя версия
not-linked = Не привязан
no-answer = Не отвечает
download = Скачать
downloading = Скачиваю...
unpacking = Распаковываю...
could-not-download = Не скачалось
mb = МБ

replays = Реплеи
worker = Воркер
settings = Настройки
search-hint = Игрок, карта, день
render = Отрендерить
get-the-map = Скачать карту
today = сегодня
coming-later = Будет доступно позже!
back-to-replays = К реплеям
no-replays = Реплеев пока нет...
drop-here = Перетащите файлы .osr в это окно.
unknown-map = Неизвестная карта
month-1 = января
month-2 = февраля
month-3 = марта
month-4 = апреля
month-5 = мая
month-6 = июня
month-7 = июля
month-8 = августа
month-9 = сентября
month-10 = октября
month-11 = ноября
month-12 = декабря
replay-read = Реплей прочитан
map-on-disk = Карта на месте
judged = Судейство сошлось
drawing = Рисую...
saving = Сохраняю...
stop = Стоп
rendered = Готово
open = Открыть
show-in-folder = Показать в папке
stopped = Остановлено
seconds-left = { $n } с
looking-up = Ищу карту...
found-on = Нашлась на { $who }
fetch-downloading = Скачиваю...
unpacking-into = Распаковываю в Dossier/Songs...
checking-hash = Сверяю хэш...
not-on-any-mirror = Нет ни на одном зеркале
try-again = Попробовать снова
look-on-device = Поискать на этом устройстве
looking-on-device = Ищу на этом устройстве...
found-on-device = Найдено на этом устройстве
nothing-on-device = На этом устройстве реплеев нет
files-label = { $n ->
    [one] файл
    [few] файла
   *[other] файлов
}
reading-replay = Читаю реплей...
judging = Сужу...
in-folder = Показать в папке
did-not-work = Не вышло · ещё раз
not-on-mirrors = Нет на зеркалах · ещё раз
reading = Читаю...
looking = Ищу...
found = Нашлась!
unpacking-map = Распаковываю...
checking-map = Сверяю...
once-more = Ещё раз
not-found = Не нашлась
of-max = из { $max }
videos = Видео
no-videos = Пока ни одного видео
render-one = Отрендерите реплей и он появится здесь.
when = когда
who-and-map = кто · карта
mods = моды
length = длина
size = размер
to-telegram = Отправить в Telegram
delete = Удалить
delete-video = Удалить видео?
to-the-bin = Файл уйдёт в корзину. Реплей и карта останутся.
keep = Оставить
rendered-notice = Отрендерено
render-failed = Рендер не завершился
map-fetched = Карта скачана
signed-in = Вход выполнен
linked-status = Привязано
not-signed-in = Вход не выполнен
stays-here = Видео остаются на этом компьютере
why-sign-in = Привяжите устройство к Telegram для открытия сетевых пользовательских функций и получите возможность стать воркером для вашей беседы.
sign-in = Войти через Telegram
sign-in-how = Откройте ЛС с ботом и запустите его. После запуска бот получит этот код и привяжет устройство.
code-lasts = Действует пять минут
waiting-confirm = Жду подтверждения…
copy-link = Скопировать ссылку
later-word = Позже
account = Аккаунт
feed = Лента
stats = Статистика
sign-out = Выйти
esc-close = Escape закрывает меню.
all-read = Всё прочитано
videos-go-to = Видео уходят в чат
build = Сборка
as-worker = как воркер
on-this-device = на этом устройстве
jobs-done = Сделано работ
replays-in-journal = Реплеев в журнале
rendered-count = Отрендерено
sent-count = Отправлено в Telegram
nothing-yet = Пока ничего
sending = Отправляю...
sent-notice = Ушло в Telegram
send-failed = Не отправилось
telegram-id = Telegram ID
close = Закрыть
map-not-fetched = Карта не скачалась
community = Сообщество
community-why = Будет доступно позже как часть экосистемы с ботом...
app-side = Приложение
bot-side = Бот
render-tile = Рендер
frames = Кадры
quality = Качество
language = Язык
look-tile = Интерфейс
scale-to-monitor = Подстраивать под монитор
scale = Масштаб
scale-auto = авто
device-tile = Устройство
scene-tile = Сцена
live-replay = Живой реплей
pause-unfocused = Пауза без фокуса
auto-flip = Листать журнал самому
sources = Источники
add-folder = Добавить папку…
exported-only = Только экспортированные реплеи
replays-count = { $n ->
    [one] реплей
    [few] реплея
   *[other] реплеев
}
scores-count = { $n ->
    [one] скор из scores.db
    [few] скора из scores.db
   *[other] скоров из scores.db
}
maps-count = { $n ->
    [one] карта
    [few] карты
   *[other] карт
}
change = Изменить
maps-and-cache = Карты и кэш
cache = кэш
clear = Очистить
builds = Сборки
check = Проверить
no-ffmpeg = ffmpeg не найден
private-chat = личный чат
group-chat = группа · бот внутри
errors = Ошибки
this-device = Это устройство
unlink = Отвязать
render-size = Размер
render-frames = Кадры
render-quality = Качество
render-cpu = Процессор
skins = Скины
own-skin = Свой скин приложения
sound = Звук
music = Музыка
hitsounds = Хитсаунды
player-sound = Плеер
rescan = Обновить
skins-found = { $n ->
    [one] найден { $n } скин
    [few] найдено { $n } скина
   *[other] найдено { $n } скинов
}
own-skin-short = Dossier Default
no-skins = В папках osu! скинов не нашлось
add-skin = Добавить скин…
skins-folder = Папка скинов
seconds-short = с
sound-on = Звук включён
sound-off = Без звука
loop-on = Повтор включён
loop-off = Повтор выключен
play = Смотреть
pause = Пауза
watch-again = Сначала
earlier-video = Предыдущее
later-video = Следующее
back-five = Назад на 5 секунд
ahead-five = Вперёд на 5 секунд
frame-back = Кадр назад
frame-ahead = Кадр вперёд
speed = Скорость
widen = Во весь экран
narrow = Свернуть
close-player = Закрыть
skin-failed = Скин не распаковался
skin-added = Скин добавлен
skins-packed = { $n ->
    [one] { $n } в .osk
   *[other] { $n } в .osk
}
no-skin-there = Скинов в папке нет
more-skins = Подробнее…
play-tile = Игровой процесс
background-dim = Затемнение
background-blur = Размытие
hud = Интерфейс
cursor-grows = Курсор на нажатия
map-sounds = Звуки карты
skin-sounds = Звуки скина
community-feed = Лента
community-people = Люди
community-boards = Рейтинг
community-titles = Титулы
community-staged = образцы данных
players = { $n ->
    [one] игрок
    [few] игрока
   *[other] игроков
}
games = { $n ->
    [one] игра
    [few] игры
   *[other] игр
}
board-pp = pp
board-accuracy = Точность
board-plays = Игры
board-hours = Время
board-score = Рейтинговые очки
board-hits = Попаданий за игру
no-title = без титула
streak = Серия
streak-days = { $n ->
    [one] { $n } день подряд
    [few] { $n } дня подряд
   *[other] { $n } дней подряд
}
hours-short = ч
global-rank = Мировой ранг
top-plays = Топ-плеи
nobody-yet = пока ни у кого
secret-title = Секретный титул
unlocked = открыто
rarity-common = Обычные
rarity-uncommon = Необычные
rarity-rare = Редкие
rarity-epic = Эпические
rarity-legendary = Легендарные
rarity-mythic = Мифические
rarity-secret = Секретные
streak-card = { $n ->
    [one] { $n } день подряд
    [few] { $n } дня подряд
   *[other] { $n } дней подряд
}
panel-news = Новости osu!
news-loading = загружаю…
news-failed = не удалось · ещё раз
news-updated = обновлено
news-unreachable = не отвечает, попробую позже
live-now = только что
minutes-ago = { $n } мин назад
    [one] и ещё { $n } правка
    [few] и ещё { $n } правки
   *[other] и ещё { $n } правок
}
channel-hint = @канал
channel-add = Добавить
people-chat = Беседа
people-game = Из игры
online = в сети
hours-ago = { $n } ч назад
days-ago = { $n ->
    [one] { $n } день назад
    [few] { $n } дня назад
   *[other] { $n } дней назад
}
read-outside = Открыть в браузере
card-grades = ОЦЕНКИ
card-no-data = Недостаточно данных
friends-link = Привяжите osu! в боте, чтобы видеть друзей
friends-relink = Перепривяжите osu! в боте (Настройки → Аккаунт), чтобы видеть друзей
community-no-group = Бот не нашёл вас ни в одной группе
community-profile = Профиль
act-telegram = В Telegram
activity-head = Активность · 13 недель
all-boards = Весь рейтинг →
best-plays = Лучшие игры
board-short-pp = PP
board-short-accuracy = Точн.
board-short-plays = Игры
board-short-hours = Часы
board-short-score = Очки
board-short-hits = Попад.
channels-head = Каналы
climb-was = было
compare = Сравнить
day-today = Сегодня
day-yesterday = Вчера
days-long = { $n ->
    [one] { $n } день
    [few] { $n } дня
   *[other] { $n } дней
}
days-short = { $n }д
filter-all = Всё
filter-plays = Игры
filter-top = Топ-плеи
filter-titles = Титулы
filter-ranks = Рейтинг
filter-news = Новости
filter-builds = Обновления
friends-head = Друзья в игре
friends-online = { $n } онлайн
grade-a = A
grade-s = S золотая
grade-sh = S серебряная (Hidden)
grade-ss = SS золотая
grade-ssh = SS серебряная (Hidden)
held-by = есть у
info-country = Страна
info-duels = Дуэли
info-since = В osu!
info-since-from = с
kind-climb = рейтинг
kind-news = новость
kind-title = титул
kind-top = топ-плей
journal-time = Время
journal-player = Игрок
journal-map = Карта
journal-accuracy = Точн.
journal-mods = Моды
journal-pp = pp
event-top = новый топ-плей
event-title = получил титул
event-climb = поднялся в таблице
less = меньше
level-short = Ур.
metric-accuracy-short = Точн.
metric-world = Мировой
more = больше
my-profile-open = Мой профиль
no-plays = без игр
of-all-grades = всех оценок
offline = не в сети
open-osu = Открыть в osu!
place-head = Место в группе
plays-n = { $n ->
    [one] { $n } игра
    [few] { $n } игры
   *[other] { $n } игр
}
spot-head = В центре внимания
spot-gain = Больше всех pp за неделю
spot-accuracy = Лучшая точность группы
spot-title = Новый титул
spot-streak = Самая длинная серия
spot-top = Первый в группе по pp
spot-you = Это вы · место по pp
streak-best-n = лучшая { $n }
title-got = получен
title-not-yet = ещё не получен
top-place = #{ $n } в личном топе
week-short = Неделя { $n }
weekday-mon = Пн
weekday-tue = Вт
weekday-wed = Ср
weekday-thu = Чт
weekday-fri = Пт
weekday-sat = Сб
weekday-sun = Вс
years = { $n ->
    [one] { $n } год
    [few] { $n } года
   *[other] { $n } лет
}
board-general = Общий
board-adaptive = Адаптивный
board-title-general = Лидерборд · Общий
board-title-adaptive = Лидерборд · Адаптивный
board-all-time = за всё время
board-week-span = неделя { $week } · { $span }
participants = { $n ->
    [one] участник
    [few] участника
   *[other] участников
}
board-sat-out = { $n } без прироста
board-total = { $value } всего
board-gap = до { $place }-го места осталось { $value }
board-new = NEW
board-no-gain = на этой неделе прироста пока ни у кого
board-collecting = идёт сбор данных — первый зачёт { $date }
board-not-played = ещё не играл на этой неделе
board-acc-unit = п.п.
minutes-short = м
in-group = в группе
stream-group = Группа
source-osu = osu!
source-channels = Каналы
highlights = Главное
feed-search = Игрок или карта
events = { $n ->
    [one] событие
    [few] события
   *[other] событий
}
news-items = { $n ->
    [one] запись
    [few] записи
   *[other] записей
}
week-leaders = Лидеры недели
last-seen = последний визит { $when }
worker-take = Брать работу у бота
worker-about = Пока переключатель включён, приложение берёт у бота реплеи участников ваших групп, рисует их в 1080p при 60 кадрах с выровненной громкостью и отправляет обратно в Telegram. Ваши собственные рендеры в это время ждут, пока воркер закончит.
worker-off = Выключен
worker-waiting = Ждёт работы
worker-resting = Ждёт, пока закончится ваш рендер
worker-offline = Нет связи с ботом
worker-taken = Взял задачу
worker-getting-replay = Скачивает реплей
worker-getting-map = Ищет карту
worker-getting-skin = Скачивает скин
worker-drawing = Рисует
worker-polishing = Выравнивает громкость
worker-sending = Отправляет видео
worker-speed = Скорость
worker-fps = { $n } кадр/с
worker-delivered = Сдано
worker-handed-back = Возвращено
worker-last-delivered = Последнее: сдано «{ $title }»
worker-last-back = Последнее: возвращено «{ $title }» — { $reason }
worker-not-paired = Приложение не привязано к боту — без этого ферма не даст работы.
worker-no-ffmpeg = Нет ffmpeg — без него видео не собрать.
worker-left = осталось { $left }
worker-busy = Воркер сейчас рисует для фермы — дождитесь, пока он закончит.
farm-head = Ферма
farm-waiting = в очереди: { $n }
farm-nobody = Сейчас никто не на связи
farm-this = это устройство
farm-rendering = рисует
farm-ready = ждёт работы
farm-resting = отдыхает
more-changes-n = и ещё { $n }
dossier-of = Досье ·
mapped-by = карта
open-map = Карта
storage = Хранилище
storage-app = само приложение —
storage-videos = Видео
storage-skins = Скины
storage-maps = Карты
storage-cache = Кэш
storage-rest = Остальное
clear-cache = Очистить кэш
prerelease = пред-релиз
update-checking = Проверяю обновления…
update-latest = Это последняя версия
update-found = Вышла
update-getting = Скачиваю
update-ready = готова к установке
update-ready-quiet = поставлю, пока тебя нет
update-ready-waits = поставлю, как только закончится работа
update-failed = Не вышло
update-moved = приложение перенесли, пока оно работало — перезапусти его, и обновление пройдёт
update-quarantined = macOS запустила Dossier из карантина: перенеси Dossier.app в «Программы», открой оттуда, и обновления заработают
update-damaged = архив пришёл повреждённым — попробуй ещё раз
update-empty = в архиве нет приложения для этой системы
update-source = Собрано из исходников — обновляется через git
update-now = Обновить
whats-new = Что нового
quiet-updates = Обновляться молча, пока меня нет
update-out = Вышло обновление
updated = Dossier обновлён
search-journal = Игрок, карта, +HDDT…
search-nothing = По этому запросу реплеев нет
reading-maps = Разбираю карты
reading-replays = Читаю реплеи
new-replay = Новый реплей
new-replays = { $n ->
    [one] новый реплей
    [few] новых реплея
   *[other] новых реплеев
}
wear-title = Надеть
take-off-title = Снять
title-not-worn = Титул не сменился
pick-map = Указать карту…
not-this-map = Это другая карта — реплей сыгран не на ней
