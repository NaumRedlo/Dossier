...
fn main() -> iced::Result {
    ...
    if let Some(at) = args.iter().position(|a| a == "--render") {
        ...
        let ask = dossier_native::render::Ask { replay: entry.path.clone(), map: map.file.clone(), map_hash: entry.map_hash.clone(), ffmpeg, out, size: dossier_native::render::SIZE, fps: dossier_native::render::FPS as u32, crf: 20 };
